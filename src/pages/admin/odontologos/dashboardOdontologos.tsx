...
export default function DasboardOdontologos() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setIsSaving(true);
      // 🌟 SOLUCIÓN: Agregamos 'isActive' y 'active' al payload para que no se pierda al editar
      const payload = {
        ...
        active: editingDoctor ? editingDoctor.isActive : true,
      };
  ...
  return (
    <AdminLayout currentPage="odontologos">
      <div className="space-y-6 animate-in fade-in duration-500 w-full max-w-full min-w-0">
        ...
        {/* Tarjetas de Estadísticas (Grilla Responsive y Premium) */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
          <div className="bg-white rounded-2xl border border-teal-100 p-5 shadow-sm transition-all hover:shadow-md flex items-center justify-between min-w-0">
            <div className="min-w-0 mr-2">
              <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1 truncate">Total Equipo</p>
              <p className="text-2xl font-black text-slate-900 truncate">{doctors.length}</p>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-teal-100 p-5 shadow-sm transition-all hover:shadow-md flex items-center justify-between min-w-0">
            <div className="min-w-0 mr-2">
              <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1 truncate">Activos</p>
          ...
          <div className="bg-white rounded-2xl border border-teal-100 p-5 shadow-sm transition-all hover:shadow-md flex items-center justify-between min-w-0">
            <div className="min-w-0 mr-2">
              <p className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-1 truncate">Especialidades</p>
              <p className="text-2xl font-black text-indigo-600 truncate">
        ...
        {/* Table Container Control Estricto */}
          ...
          {loading ? (
            ...
            <div className="text-center py-20 px-4">
              ...
              <button
                ...
                className="mt-4 px-6 py-2.5 bg-teal-50 text-teal-700 font-bold rounded-xl hover:bg-teal-100 transition-colors flex items-center justify-center gap-2 mx-auto shadow-sm"
              >
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-teal-100 bg-gradient-to-r from-cyan-50 to-teal-50">
                    ...
                    <th className="px-5 py-4 text-xs font-bold text-slate-800 uppercase tracking-wider whitespace-nowrap">Especialidad</th>
                  ...
                  {filteredDoctors.map((doctor) => (
                    <tr key={doctor.id} className="hover:bg-cyan-50/50 transition-colors">
                      ...
                      <td className="px-5 py-4 text-sm whitespace-nowrap">
                        <span className="px-3 py-1.5 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded-lg text-[11px] font-black tracking-wider uppercase">
                          {doctor.specialty}
                        </span>
                      </td>
                      <td className="px-5 py-4 text-sm whitespace-nowrap">
                        <span
                          className={`px-3 py-1.5 rounded-lg text-[11px] font-black tracking-wider uppercase border ${
                            doctor.isActive
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  required
                  disabled={isSaving}
                  className="w-full px-4 py-3 border border-slate-200 rounded-xl focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-200 transition-all bg-white disabled:opacity-60 disabled:bg-slate-50 cursor-pointer text-sm font-medium text-slate-700 shadow-sm"
                >
              ...
              {!editingDoctor && (
                <>
                  <div className="border-t border-slate-100 pt-5 mt-5">
                    <p className="text-[11px] font-black text-teal-600 uppercase tracking-wider mb-4 flex items-center gap-1.5">
                      <Stethoscope className="w-4 h-4 shrink-0" /> Datos de Acceso al Sistema
                    </p>
                  </div>
      ...
      {isConfirmOpen && doctorToToggle && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm rounded-3xl bg-white p-6 text-center shadow-2xl animate-in zoom-in-95 duration-200">
            <div className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full mb-4 ${doctorToToggle.isActive ? 'bg-amber-100 border-amber-50 border-4' : 'bg-emerald-100 border-emerald-50 border-4'}`}>
